...
`, localPort, filepath.Join(tmpDir, "tls.crt"), filepath.Join(tmpDir, "tls.key"))
	...
	It("should handle forceAggregation when CRDs are installed first", func() {
		By("enabling forceAggregation on the APIShard")
		cmd := exec.Command("kubectl", "patch", "apishard", shardName,
			"--type=merge", "-p", `{"spec":{"forceAggregation":true}}`)
		_, err := run(cmd)
		Expect(err).NotTo(HaveOccurred(), "Failed to enable forceAggregation")

		By("installing the dummy CRD on the primary (with forceAggregation enabled)")
		cmd = exec.Command("kubectl", "apply", "-f", filepath.Join(testdataDir, "dummy_crd.yaml"))
		_, err = run(cmd)
		Expect(err).NotTo(HaveOccurred(), "Failed to install CRD on primary")
		...
		By("verifying phase is NOT Blocked (force mode prevents blocking)")
		cmd = exec.Command("kubectl", "get", "apishard", shardName,
			"-o", "jsonpath={.status.phase}")
		output, err := run(cmd)
		Expect(err).NotTo(HaveOccurred())
		...
		By("verifying the APIService has automanaged=false label")
		Eventually(func(g Gomega) {
		...
		By("verifying aggregation still works (data visible from secondary)")
		Eventually(func(g Gomega) {
			cmd := exec.Command("kubectl", "get", "widgets.example.com",
				widgetName, "-n", widgetNamespace,
				"-o", "jsonpath={.spec.message}")
			output, err := run(cmd)
			g.Expect(err).NotTo(HaveOccurred())
			g.Expect(output).To(Equal("hello from e2e test"),
				"Widget data should still be accessible via aggregation")
		}, 30*time.Second, 5*time.Second).Should(Succeed())

		By("cleaning up: delete CRD and disable forceAggregation")
		cmd = exec.Command("kubectl", "delete", "crd", "widgets.example.com", "--ignore-not-found")
		_, _ = run(cmd)
		cmd = exec.Command("kubectl", "patch", "apishard", shardName,
			"--type=merge", "-p", `{"spec":{"forceAggregation":false}}`)
		_, _ = run(cmd)

		By("waiting for phase to return to Ready")
		Eventually(func(g Gomega) {
			cmd := exec.Command("kubectl", "get", "apishard", shardName,
				"-o", "jsonpath={.status.phase}")
			output, err := run(cmd)
